...
namespace FileExplorer
{
    public partial class Form1 : Form
    {
        ...
        private void btnFiltrele_Click(object sender, EventArgs e)
        {
            ...
            if (cmbFiltre.SelectedIndex > 0)
            {
                seciliUzanti = cmbFiltre.SelectedItem.ToString();
                seciliUzanti = seciliUzanti.Substring(1, seciliUzanti.IndexOf('(') - 2);
            ...
            int rowSayisi = listView1.Items.Count;
            double toplamDosyaBoyutu = 0;

            for (int i=0; i< rowSayisi; i++)
            {
                toplamDosyaBoyutu += Convert.ToDouble(listView1.Items[i].SubItems[1].Text);
            }

            toolStripStatusLabel1.Text = string.Format("{0} adet dosya, {1} boyut", rowSayisi, this.ToplamDosyaBoyutuCevirici(toplamDosyaBoyutu));

        }

        string ToplamDosyaBoyutuCevirici(double toplamDosyaBoyutu)
        {
            string sonuc = "";
        ...
        void DosyalariDoldur(string path)
        {
            ...
            Dictionary<string, int> uzantilar = new Dictionary<string, int>();
            foreach (FileInfo fiDosya in dosyalar)
            {
                ...
                if (!uzantilar.ContainsKey(uzanti))
                    uzantilar.Add(uzanti, 1);
                else
                    uzantilar[uzanti]++;
            ...
            if (!textChanged)
            {
                ...
                cmbFiltre.Items.Add("*.*");
                foreach (string a in uzantilar.Keys)
                {
                    int extValue = 0;
                    uzantilar.TryGetValue(a, out extValue);
                    string s = a + " (" + extValue + ")";
            ...
            toolStripStatusLabel1.Text = string.Format("{0} adet dosya, {1} boyut", dosyalar.Length, this.ToplamDosyaBoyutuCevirici(toplamDosyaBoyutu));
        }
